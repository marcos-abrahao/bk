...
For i:=1 to Len(aUser)
   ...
   nLin+=1
   @ nLin,000 pSay "Tipo de Impressao...............: "+aTipoImp[aUser[i][02][08]+1] // Tipo de Impressao
   nLin+=1
   @ nLin,000 pSay "Formato de Impressao............: "+aFormImp[aUser[i][02][09]+1] // Formato
   nLin+=1
   @ nLin,000 pSay "Ambiente de Impressao...........: "+aAmbImp[aUser[i][02][10]+1] // Ambiente
   nLin+=2
   ...
   For k:=1 to Len(aUser[i][01][10])
      ...
      //PswSeek(cGp,.F.)
      //PswRet()
      //aGroup := Clone(aAllGroup,nGp)  
      
      AADD(aGroups,cGp)
      aGroup := FWGrpParam(cGp)
   ...
   If mv_par09==1
      ...
      For k:=1 to LEN(aAcessos)
      	 lAcesso := .F.
      	 //For m:= 1 TO LEN(aGroups)
      	 //	If Substr(aGroups[m][02][5],k,1) == "S"
      	 //		lAcesso := .T.
      	 //	EndIf
      	 //Next
      	 For m:= 1 TO LEN(aGroups)
      	 	cAcesso := FWGrpAcess(aGroups[m])
      	 	If SUBSTR(cAcesso,K,1) == "S"   //Substr(aGroups[m][02][5],k,1) == "S"
      	 		lAcesso := .T.
      	 	EndIf
      	 Next
      
         If Substr(aUser[i][02][5],k,1) == "S" .OR. lAcesso
            fCabec(@nLin,55)
            @ nLin,aColAcess[nCol] pSay aAcessos[k]+" "+IIF(lAcesso,"G","U")
            If nCol==3
               nCol:=1
               nLin+=1
            Else
...
aReturn := {"Excluir Produtos             ",;
            ...
            "Manuten Aus Temp Todas       ",;
            "UManutn Aus Temp Usuario     ",;
            "Formação de preço            ",;
            "Gravar resposta parametros   ",;
            "Configurar consulta F3       ",;
            "Permite Alterar conf. impre. ",;
            "Gerar rel em disco local     ",;
            "Gerar rel no servidor        ",;
            "Inclui solic. de compras     ",;
            "Mbrowse - visulalisa ourtas f",;
            "Mbrowse - Edita reg outras fi",;
            "MBroses permite uso de filtro",;
            "F3 - Permite o uso de filtro ",;
            "MBroese permite conf colunas ",;
            "Altera orçamento aprovado    ",;
            "Revisa Orçamento aprovado    ",;
            "Usa impressora no server     ",;
            "Usa impressora client        ",;
            "Agendar processos/Relatorios ",;
            "Processo identicos na MDI    ",;
            "Cad Cli no catalogo de email ",;
            "Cad For no catalogo de email ",;
            "Impr informacoes personalizad",;
            "Resposta param MV_WFMESSE    ",;
            "Aprovar rej. pré estrutura   ",;
            "criar estrutura com base pré ",;
            "Gerir Etapas                 ",;
            "Gerir Despesas               ",;
            "Liberar despesa para fatur.  ",;
            "Lib. ped. vendas credito     ",;
            "Lib Ped. vendas estoque      ",;
            "Habilitar opção executar     ",;
            "Acesso via activeX           ",;
            "Excluir bens                 ",;
            "Rateio do item por CC        ",;
            "Alterar cad cliente          ",;
            "Excluir cad. cliente         ",;
            "Habilitar filtros rel.       ",;
            "Contatos no cat. de e-mail   ",;
            "Criar formulas nos rel.      ",;
            "Acesso ao cad. de lotes      ",;
            "Gravar resposta param por emp",;
            "Manutenção no repositorio ima",;
            "Criar rel. personalizaveis   ",;
            "Permissao para usar TOII     ",;
            "Acesso ao siga RPM           ",;
            "Maisc/Minusc na consulta pad ",;
            "Valida acesso do grp emp/fil ",;
            "Acessa base inst no cad tec  ",;
            "Desabilita função usu. menu  ",;
            "Impressão em planilha        ",;
            "Acesso a scripts confidencia ",;
            "Qualificação de suspects.    ",;
            "Execução de scripts dinamicos",;
            "MDI permite encerrar pelo X  ",;
            "Permite utilizar o WalkThru  ",;
            "Geração de forecast          ",;
            "Execução de marshups         ",;
            "permite exportar pla PMS exce",;
            "Gravar filtro do browse/emp/f",;
            "Exportar telas para excel    ",;
            "Se Adm pode usar sigacfg     ",;
            "Se adm pode usar MPSDU       ",;
            "Se acessa APSDU é r/w        ",;
            "Acesso a inscr nos eventos ev",;
            "MBrowse permite util. localiz",;
            "Visulalização via F3         ",;
            "Exclui Purchase order        ",;
            "Alterar Purchase order       ",;
            "Exclui solic de import       ",;
            "Alterar solici de import.    ",;
            "Excluir desembaraço          ",;
            "Alterar desembaraço          ",;
            "Incluir agenda medica        ",;
            "Alterar agenda medica        ",;
            "Excluir agenda médica        ",;
            "Acesso a formulas            ",;
            "Utilizar config imp TMSPrint ",;
            "MBrowse habilita impressão   ",;
            "Acesso via smartclient HTML  ",;
            "Grava conf browse p/ emp/fil ",;
            "Permite efetuar manuais LC   "}
...
Local aArea	    := GetArea()
Local cPerg		:= PAD("MAPAUS",LEN(SX1->X1_GRUPO))
Local aRegs		:= {}
Local i
