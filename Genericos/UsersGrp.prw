...
If EMPTY(aPswBrw)
	...
	//Processa({|| aData := U_BkAllUsr(),aEval(aData,{|cVal,nInd| iif(!aData[nInd][1][17],Aadd(aPswBrw,{aData[nInd][1][1],aData[nInd][1][2],If(aData[nInd][1][1] <> "000000",aData[nInd][1][4],""),aData[nInd][1][14]}),x:=0) }) },"Carregando usuários")
	//Processa({|| aPswBrw := U_BkUsrGrp("000007/000004/000003/000000/000010")},"Aguarde...","Carregando usuários",.F.)

	oProcess := MsNewProcess():New( { | lEnd | oProcess:SetRegua1( 8 ),oProcess:IncRegua1("Filtrando usuários..."),aPswBrw := U_BkUsrGrp("000007/000004/000003/000000/000010") }, "Aguarde..", "Carregando usuários...", .F. )
	oProcess:Activate()
