...
Função para editar o MV_XXUSERS
@author Atilio
@since 16/05/2016
@version 1.0
    @example
...
Usuários contidos em um parâmetro
@author Atilio
@since 16/05/2016
@version 1.0
    @param cParam, Caracter, Códigos dos usuários contidos no parâmetro
...
User Function zParUsr(cParam, cCarSep, lUsrFil)
    ...
    If Empty(cParam)
        MsgStop("Parâmetro em branco!", "Atenção")
        Return
    EndIf
...
Static Function fPopula()
    ...
    For nAtu := 1 To Len(aUsers)
        ...
        RecLock(cAliasTmp, .T.)
            ...
            XX_ATIVO    := IIF(!aUsers[nAtu][1][POS_BLOQ],"Sim","Não")
        (cAliasTmp)->(MsUnlock())
    Next
...
Static Function fGetMkA(cMarca,cCarSep)
    ...
    If lChecado
        ...
        If Len(Alltrim(cGetReto) + cChave) > nTamFim
            MsgAlert("Tamanho do Parâmetro Excedido!", "Atenção")
            lFalhou := .T.
