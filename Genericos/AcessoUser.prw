...
Define MsDialog oDlg Title "Acesso de Usuários" From oMainWnd:nTop+125,oMainWnd:nLeft+5 To oMainWnd:nBottom-60,oMainWnd:nRight-10 Of oMainWnd Pixel
    *
    oPanel = TPanel():New(nLin,nCol-5,"Colunas a serem impressas",oDlg,,.F.,,,,185,45,.F.,.T.)
    nLin += 15
...
If cTipo == "USERS"
   ...
   aAdd(aTbCpos,{"DEPART"   ,,"Departamento",""} )
   //
...
For i := 2 To nTamUser
   ...
   If !aUsers[i][1][17] .AND. cUserDe <= aUsers[i][1][1] .AND. cUserAte >= aUsers[i][1][1]
   			...
   			For j:=1 To nTamMod
         		...
         		If SubStr(aGrups[j],3,1) != "X" .AND. cModDe <= SubStr(aGrups[j],1,2) .AND. cModAte >= SubStr(aGrups[j],1,2)
            		...
            		If lAppMod
            			...
            			WKMODULOS->WKMARCA   := If(lMarca,cMarca,"")
            			WKMODULOS->CODUSER   := aUsers[i][1][1] //Código do user
             			WKMODULOS->CODMODULO:= SubStr(aGrups[j],1,2) //Código do módulo
            			WKMODULOS->MODULO    := retModulo(Val(WKMODULOS->CODMODULO)) //função que retorna a descrição do módulo de acordo com o código passado.
            			lAppUser := .T.
            		EndIf
        	 	EndIf
      ...
      If lAppUser
         ...
         WKUSERS->DEPART  := aUsers[i][1][12] //Departamento
   ...
   While !Ft_FEof() 
      ...
      ElseIf "MENUITEM " $ Upper(cAux)
         ...
         If AllTrim(cRotDe) <= AllTrim(cFuncao) .AND. AllTrim(cRotAte) >= AllTrim(cFuncao)
            ...
            WKACESSO->CODUSMOD  := aUsers[i][1][1]+SubStr(cModulo,1,2) //Código do user + Código do módulo
            WKACESSO->USER      := aUsers[i][1][4] //Nome do user
            WKACESSO->DEPART    := aUsers[i][1][12] //Departamento
            WKACESSO->MODULO    := retModulo(Val(SubStr(cModulo,1,2))) //Nome do Módulo
            WKACESSO->MENU      := cMenu
            WKACESSO->SUBMENU   := cSubMenu
            WKACESSO->ROTINA    := cRotina
            WKACESSO->ACESSO    := cAcesso
            WKACESSO->TIPO      := cTPMNU
