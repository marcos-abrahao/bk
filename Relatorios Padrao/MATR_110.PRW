...
// BK - Abrir dialogo para digitação do endereço de entrega
If lAuto
	dbSelectArea("SC7")
	dbGoto(nReg)
    cEndEnt := SM0->M0_ENDENT+"  "+Rtrim(SM0->M0_CIDENT)+"  - "+SM0->M0_ESTENT+" - "+STR0009+" "+Trans(Alltrim(SM0->M0_CEPENT),PesqPict("SA2","A2_CEP"))
	IF SC1->(dbSeek(xFilial("SC1")+SC7->C7_NUMSC+SC7->C7_ITEMSC))
	   IF !EMPTY(SC1->C1_XXENDEN)
		cEndEnt := SC1->C1_XXENDEN
	   ENDIF
	ENDIF
	BKALTENT()
EndIf
...
While !oReport:Cancel() .And. !SC7->(Eof()) .And. SC7->C7_FILIAL == xFilial("SC7") .And. SC7->C7_NUM >= mv_par01 .And. SC7->C7_NUM <= mv_par02
	...
	For nVias := 1 to mv_par09
        ...
        */   
        ...
        // BK - Endereço de entrega - pegar do C1_XXENDEN
        If lAuto .AND. !EMPTY(cEndEnt)
        	MV_PAR13 := cEndEnt
        EndIf
        
 		If Empty(MV_PAR13) //"Local de Entrega  : " 
		   cEndEnt := ALLTRIM(SM0->M0_ENDENT)+" "+Rtrim(SM0->M0_CIDENT)+" - "+SM0->M0_ESTENT+" "+Trans(Alltrim(SM0->M0_CEPENT),PesqPict("SA2","A2_CEP"))
           //cEndEnt := SM0->M0_ENDENT+"  "+Rtrim(SM0->M0_CIDENT)+"  - "+SM0->M0_ESTENT+" - "+STR0009+" "+Trans(Alltrim(SM0->M0_CEPENT),PesqPict("SA2","A2_CEP"))
	       IF SC1->(dbSeek(xFilial("SC1")+SC7->C7_NUMSC+SC7->C7_ITEMSC))
	           IF !EMPTY(SC1->C1_XXENDEN)
	           		cEndEnt := SC1->C1_XXENDEN
	           ENDIF
	        ENDIF
			oReport:PrintText(STR0008 + cEndEnt,, 050 )
		Else
			oReport:PrintText(STR0008 + mv_par13,, 050 ) //"Local de Entrega  : " imprime o endereco digitado na pergunte
		Endif

		SM0->(dbGoto(nRecnoSM0))
		oReport:PrintText(STR0010 + ALLTRIM(SM0->M0_ENDCOB)+" "+Rtrim(SM0->M0_CIDCOB)+" - "+SM0->M0_ESTCOB+" "+Trans(Alltrim(SM0->M0_CEPCOB),PesqPict("SA2","A2_CEP")),, 050 )
...
//		oReport:PrintText(cObs08,,050 )
		...
		oReport:PrintText(cObs07,nLinPC2,650 )
		oReport:SkipLine()
		oReport:PrintText(cObs08,,650 )
		oReport:PrintText(cObs09,,650 )
//		oReport:SkipLine()
        ...
        //BK IMPRIME OBS
        _cXXOBS := ""
		FOR nI:= 1 to MLCOUNT(SC7->C7_XXOBS,80)
			_cXXOBS := TRIM(MEMOLINE(SC7->C7_XXOBS,80,nI))
			IF !EMPTY(_cXXOBS) 
				oReport:PrintText(_cXXOBS,,050)
			ENDIF
		NEXT
...
If dbSeek(xFilial("SC7")+cPedido+cItemDe+Alltrim(cSequen))
	...
	While !Eof() .AND. SC7->C7_FILIAL+SC7->C7_NUM == xFilial("SC7")+cPedido .AND. ;
		...
		While !EOF() .AND. (X3_ARQUIVO == 'SC7')
			...
			// Excluido o campo C7_OPER em 4/11/19 provisioriamente para não dar erro
			If "MAFISREF" $ cValid .AND. !(TRIM(SX3->X3_CAMPO) $ "C7_OPER") 
...
//Local lAltVal  := .F.
Local cTitulo2 := "BKALTENT - Dados complementares - Pedido de Compras: "+SC7->C7_NUM
	
oOk := LoadBitmap( GetResources(), "LBTIK" )
oNo := LoadBitmap( GetResources(), "LBNO" )

DO WHILE .T.	
	DEFINE MSDIALOG oDlg TITLE cTitulo2 FROM 000,000 TO 140,450 PIXEL 
		
	//@ 000,000 MSPANEL oPanelLeft OF oDlg SIZE 220,125
	//oPanelLeft:Align := CONTROL_ALIGN_LEFT
	
	@ 040, 005 SAY "Endereço de Entrega: "  SIZE 65,07 OF oDlg PIXEL
	@ 040, 070 MSGET cEndEnt SIZE 150,10 VALID !EMPTY(cEndEnt) OF oDlg PIXEL HASBUTTON
