...
IF cBanc $ "104/237"
	...
	ENDIF
	IF !cBcoArq $ "104/237"
		MsgStop("Banco informado layout indisponํvel!!", "Aten็ใo")
		lOk:= .F.
	ENDIF
	IF cBcoArq <> cBanc .AND. cbanc <> "237"
		MsgStop("Banco informado diferente do Arquivo informado!!", "Aten็ใo")
		lOk:= .F.
	ELSE
		IF cBanc == "104"
			...
			ELSE
				MsgStop("Arquivo selecionado nใo ้ retorno. Verifique!!", "Aten็ใo")
				lOk:= .F.
			ENDIF
		ENDIF
	ENDIF
...
IF LEN(aCtrId) == 0
	MSGSTOP("Nใo hแ dados "+IIF(SUBSTR(cSim,1,1) == "S"," de FURNAS","")+IIF(SUBSTR(cSim,1,1) == "L"," de FURNAS LOTE BK","")+" no arquivo. Verifique!")
	RETURN NIL
ENDIF
...
DEFINE MSDIALOG oDlg TITLE "Selecione o boleto desejado para impressใo" FROM 000,000 TO 500,830 PIXEL 
...
@ 022, 005 LISTBOX oListID FIELDS HEADER "","Prontuario","Nome","CPF","Agencia","DA","Conta Corrente","DC","Dt.PGTO","Valor","Status","Ocorr๊ncia(s)" SIZE 410,220 OF oPanelLeft PIXEL 
   ...
   IF aCtrId[_XI,1]
	  ...
	  @ nLin,nPos PSAY DTOC(aCtrId[_XI,9])
	   ...
	   IF !EMPTY(aCtrId[_XI,12])
	   		nLin++
	  	 	nPos := 08
	   		@ nLin,nPos PSAY "Ocorr๊ncia(s): "+aCtrId[_XI,12] 
	   ENDIF
