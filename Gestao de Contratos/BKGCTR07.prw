...
/*
 SELECT DISTINCT CN9_CLIENT AS XX_CLIENTE,CN9_LOJACL AS XX_LOJA,CNF_CONTRA,CNF_REVISA,CNF_COMPET,
    CASE WHEN CN9_SITUAC = '05' THEN CNF_VLPREV ELSE CNF_VLREAL END AS CNF_VLPREV,
    CASE WHEN CN9_SITUAC = '05' THEN CNF_SALDO  ELSE 0 END AS CNF_SALDO, 
    CTT_DESC01, 
    CNA_NUMERO,CNA_XXMUN, 
    CND_NUMMED, 
    C6_NUM, 
    (SELECT SUM(CNR_VALOR) FROM CNR010 CNR WHERE CND_NUMMED = CNR_NUMMED
         AND  CNR_FILIAL = CND_FILIAL AND  CNR.D_E_L_E_T_ = ' ' AND CNR_TIPO = '1') AS XX_BONIF,
    (SELECT SUM(CNR_VALOR) FROM CNR010 CNR WHERE CND_NUMMED = CNR_NUMMED
         AND  CNR_FILIAL = CND_FILIAL AND  CNR.D_E_L_E_T_ = ' ' AND CNR_TIPO = '2') AS XX_MULTA,
    F2_DOC,F2_EMISSAO,F2_VALFAT,F2_VALIRRF,F2_VALINSS,F2_VALPIS,F2_VALCOFI,F2_VALCSLL,F2_RECISS,F2_VALISS, 
    (SELECT TOP 1 E1_VENCTO FROM SE1010 SE1 WHERE E1_PREFIXO = F2_SERIE AND E1_NUM = F2_DOC
        AND  E1_FILIAL = '  '  AND  SE1.D_E_L_E_T_ = ' ') AS XX_VENCTO, 
    (SELECT TOP 1 E1_VENCORI FROM SE1010 SE1 WHERE E1_PREFIXO = F2_SERIE AND E1_NUM = F2_DOC
        AND  E1_FILIAL = '  '  AND  SE1.D_E_L_E_T_ = ' ') AS XX_VENCORI, 
    (SELECT TOP 1 E1_BAIXA FROM SE1010 SE1 WHERE E1_PREFIXO = F2_SERIE AND E1_NUM = F2_DOC
        AND  E1_FILIAL = '  '  AND  SE1.D_E_L_E_T_ = ' ') AS XX_BAIXA, 
    (SELECT SUM(E5_VALOR) FROM SE5010 SE5 WHERE E5_PREFIXO = F2_SERIE AND E5_NUMERO = F2_DOC  AND E5_TIPO = 'NF' AND  E5_CLIFOR = F2_CLIENTE AND E5_LOJA = F2_LOJA AND E5_TIPODOC = 'DC' AND E5_RECPAG = 'R' AND E5_SITUACA <> 'C' 
      AND  E5_FILIAL = '  '  AND  SE5.D_E_L_E_T_ = ' ') AS XX_E5DESC, 
    (SELECT SUM(E5_VALOR) FROM SE5010 SE5 WHERE E5_PREFIXO = F2_SERIE AND E5_NUMERO = F2_DOC  AND E5_TIPO = 'NF' AND  E5_CLIFOR = F2_CLIENTE AND E5_LOJA = F2_LOJA AND E5_TIPODOC IN ('MT','JR','CM') AND E5_RECPAG = 'R' AND E5_SITUACA <> 'C' 
      AND  E5_FILIAL = '  '  AND  SE5.D_E_L_E_T_ = ' ') AS XX_E5MULTA 
 FROM CNF010 CNF
 INNER JOIN CN9010 CN9 ON CN9_NUMERO = CNF_CONTRA AND CN9_REVISA = CNF_REVISA AND CN9_SITUAC <> '10' AND CN9_SITUAC <> '09' 
      AND  CN9_FILIAL = '01' AND  CN9.D_E_L_E_T_ = ' '
 LEFT JOIN CTT010 CTT ON CTT_CUSTO = CNF_CONTRA
      AND  CTT_FILIAL = '  ' AND  CTT.D_E_L_E_T_ = ' '
 LEFT JOIN CNA010 CNA ON CNA_CRONOG = CNF_NUMERO AND CNA_REVISA = CNF_REVISA
      AND  CNA_FILIAL = '01' AND  CNA.D_E_L_E_T_ = ' '
 LEFT JOIN CND010 CND ON CND_CONTRA = CNF_CONTRA AND CND_COMPET = CNF_COMPET AND CND_PARCEL = CNF_PARCEL AND CNA_NUMERO = CND_NUMERO AND CND_REVISA = CNF_REVISA
      AND  CND_FILCTR = '01' AND  CND.D_E_L_E_T_ = ' '
 LEFT JOIN SC6010 SC6 ON CND_PEDIDO = C6_NUM
      AND  C6_FILIAL = CND_FILIAL  AND  SC6.D_E_L_E_T_ = ' '
 LEFT JOIN SF2010 SF2 ON C6_SERIE = F2_SERIE AND C6_NOTA = F2_DOC
      AND  F2_FILIAL = CND_FILIAL  AND  SF2.D_E_L_E_T_ = ' '
 WHERE CNF_FILIAL = '01' AND  CNF.D_E_L_E_T_ = ' '
 AND SUBSTRING(F2_EMISSAO,1,6) = '202005' 
 UNION ALL 
 SELECT DISTINCT F2_CLIENTE AS XX_CLIENTE,F2_LOJA AS XX_LOJA,
        CASE WHEN (SELECT TOP 1 C5_ESPECI1 FROM SC6010 SC6 INNER JOIN SC5010 SC5 ON C5_FILIAL = C6_FILIAL AND C6_NUM = C5_NUM AND C6_SERIE = F2_SERIE AND C6_NOTA = F2_DOC AND SC6.D_E_L_E_T_ = ' ' AND SC5.D_E_L_E_T_ = ' ')  = ' ' THEN 'XXXXXXXXXX' ELSE (SELECT TOP 1 C5_ESPECI1 FROM SC6010 SC6 INNER JOIN SC5010 SC5 ON C5_FILIAL = C6_FILIAL AND C6_NUM = C5_NUM AND C6_SERIE = F2_SERIE AND C6_NOTA = F2_DOC AND SC6.D_E_L_E_T_ = ' ' AND SC5.D_E_L_E_T_ = ' ')  END,
        ' ',' ',0,0,         A1_NOME,         ' ',' ',         ' ',         ' ',         0,0, 
        F2_DOC,F2_EMISSAO,F2_VALFAT,F2_VALIRRF,F2_VALINSS,F2_VALPIS,F2_VALCOFI,F2_VALCSLL,F2_RECISS,F2_VALISS, 
        (SELECT TOP 1 E1_VENCTO FROM SE1010 SE1 WHERE E1_PREFIXO = F2_SERIE AND E1_NUM = F2_DOC
            AND  E1_FILIAL = '  '  AND  SE1.D_E_L_E_T_ = ' ') AS XX_VENCTO, 
        ...
        (SELECT SUM(E5_VALOR) FROM SE5010 SE5 WHERE E5_PREFIXO = F2_SERIE AND E5_NUMERO = F2_DOC  AND E5_TIPO = 'NF' AND  E5_CLIFOR = F2_CLIENTE AND E5_LOJA = F2_LOJA AND E5_TIPODOC IN ('MT','JR','CM') AND E5_RECPAG = 'R' AND E5_SITUACA <> 'C' 
            AND  E5_FILIAL = '  '  AND  SE5.D_E_L_E_T_ = ' ') AS XX_E5MULTA 
 FROM SF2010 SF2
 LEFT JOIN SA1010 SA1 ON F2_CLIENTE = A1_COD AND F2_LOJA = A1_LOJA
      AND  A1_FILIAL = '  ' AND  SA1.D_E_L_E_T_ = ' '
 WHERE ((SELECT TOP 1 C5_MDCONTR FROM SC6010 SC6 INNER JOIN SC5010 SC5 ON C6_FILIAL = '01' AND C5_FILIAL = C6_FILIAL AND C6_NUM = C5_NUM AND C6_SERIE = F2_SERIE AND C6_NOTA = F2_DOC AND SC6.D_E_L_E_T_ = ' ' AND SC5.D_E_L_E_T_ = ' ')  = ' ' OR (SELECT TOP 1 C5_MDCONTR FROM SC6010 SC6 INNER JOIN SC5010 SC5 ON C6_FILIAL = '01' AND C5_FILIAL = C6_FILIAL AND C6_NUM = C5_NUM AND C6_SERIE = F2_SERIE AND C6_NOTA = F2_DOC AND SC6.D_E_L_E_T_ = ' ' AND SC5.D_E_L_E_T_ = ' ')  IS NULL ) 
      AND SUBSTRING(F2_EMISSAO,1,6) = '202005'       AND F2_FILIAL = '01' AND SF2.D_E_L_E_T_ = ' '
...
//cQuery += "      AND  CND_FILCTR = '"+xFilial("CN9")+"' AND  CND.D_E_L_E_T_ = ' '"+ CRLF
