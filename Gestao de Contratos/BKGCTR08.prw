...
±±ºPrograma  ³ BKGCTR08 º Autor ³ Marcos B. Abrahão         Data ³19/01/12º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDescricao ³ Mapa de Mediçoes c/ historico                              º±±
...
cQuery := " SELECT DISTINCT CNF_CONTRA,CNF_REVISA,CNF_COMPET,"
...
cQuery += "    (SELECT SUM(CNR_VALOR) FROM "+RETSQLNAME("CNR")+" CNR "
cQuery += "         INNER JOIN "+RETSQLNAME("CND")+ " CNDB ON CNDB.CND_NUMMED = CNR_NUMMED AND"
cQuery += "                      SUBSTRING(CNDB.CND_COMPET,4,4)+SUBSTRING(CNDB.CND_COMPET,1,2) < '"+cAnoComp+cMesComp+"'" 
cQuery += "                      AND CNDB.CND_CONTRA = CNF_CONTRA AND CNDB.CND_REVISA = CNF_REVISA "
//cQuery += "                      AND CNDB.CND_PARCEL = CNF_PARCEL "
cQuery += "                      AND CNDB.CND_FILIAL = '"+xFilial("CND")+"' AND  CNDB.D_E_L_E_T_ = ' '"
cQuery += "         WHERE CNR_FILIAL = '"+xFilial("CNR")+"' AND  CNR.D_E_L_E_T_ = ' ' AND CNR_TIPO = '1') AS XX_BONANT,"
...
//cQuery += "                      AND CNDB.CND_PARCEL = CNF_PARCEL "
cQuery += "                      AND CNDB.CND_FILIAL = '"+xFilial("CND")+"' AND  CNDB.D_E_L_E_T_ = ' '"
cQuery += "         WHERE CNR_FILIAL = '"+xFilial("CNR")+"' AND  CNR.D_E_L_E_T_ = ' ' AND CNR_TIPO = '2') AS XX_MULANT,"
...
cQuery += " INNER JOIN "+RETSQLNAME("CN9")+ " CN9 ON CN9_NUMERO = CNF_CONTRA AND CN9_REVISA = CNF_REVISA AND CN9.CN9_SITUAC NOT IN ('01','02','08','09','10') "
...
//cQuery += "        ' ', "      // C6_NUM
cQuery += "        ' ', "      // CNF_PARCEL
cQuery += "        0,0,0,0, "      // XX_BONIF,XX_MULTA
cQuery += "        F2_DOC,F2_EMISSAO,F2_VALFAT,F2_VALIRRF,F2_VALINSS,F2_VALPIS,F2_VALCOFI,F2_VALCSLL,F2_RECISS,F2_VALISS, " 
cQuery += "        (SELECT TOP 1 E1_VENCTO FROM "+RETSQLNAME("SE1")+" SE1 WHERE E1_PREFIXO = F2_SERIE AND E1_NUM = F2_DOC"
cQuery += "            AND  E1_FILIAL = '"+xFilial("SE1")+"'  AND  SE1.D_E_L_E_T_ = ' ') AS XX_VENCTO "
...
//u_QryToCsv("QTMP","BKGCTR08A",{cQuery},.F.)
