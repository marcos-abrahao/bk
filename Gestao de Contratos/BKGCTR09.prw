...
FOR _nI := 1 TO nPeriodo
    ...
    IF !EMPTY(cContrCli)
    	IF nAgrupar == 1
    		cQuery += " AND CN9_XCLIEN ='"+ALLTRIM(cContrCli)+"'"
    	ELSEIF nAgrupar == 2
    		cQuery += " AND CNF_CONTRA ='"+ALLTRIM(cContrCli)+"'"
    	ENDIF
    ENDIF	
	cQuery += " GROUP BY CNF_CONTRA,CN9_REVISA,CN9_SITUAC,CN9_XCLIEN,CN9_NOMCLI,CTT_DESC01,CN9_XXDESC,CNF_COMPET ORDER BY CNF_CONTRA,CN9_REVISA,CN9_SITUAC,CN9_NOMCLI,CN9_XXDESC,CNF_COMPET"
	...
	DO WHILE QTMP->(!EOF())
        ...
        IF nAgrupar < 3
        	IF nAgrupar == 1
        	   cCliCod	:= ALLTRIM(QTMP->CN9_XCLIENE)
