...
ฑฑบFuncao    ณBKCOMA02  บAutor  ณ Marcos B. Abrahใo  บ Data ณ 29/09/2009  บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDescricao ณ Sele็ใo de NF - Integra็ใo VT/VR                           บฑฑ
...
"X"- Em sele็ใo, na tela do financeiro: RH nใo pode manipular
"S"- Titulos gerados no financeiro: financeiro pode excluir,RH nใo pode manipular
"D"- Titulos excluํdos no financeiro: RH pode manipular
...
dbGoTop()
IF BOF() .OR. EOF()
	MsgStop("Nใo ha VT/VR gerados", "Aten็ใo")
	RestArea(aAreaIni)
	Return
ENDIF
...
IF nStatusX > 0
    MsgStop("Integra็ใo jแ iniciada por outra se็ใo", "Aten็ใo")
	IF __cUserId <> "000000"
	   MsgStop("Verifique se hแ algum usuario processando a integra็ใo, caso contrแrio comunique o administrador do sistema", "Aten็ใo")
    ELSE
...
If Empty(aCtrId)
	MsgStop("Nใo hแ NFs disponํveis", "Aten็ใo")
	RestArea(aAreaIni)
	Return
EndIf
...
DEFINE MSDIALOG oDlg TITLE "Sele็ใo de NF - Integra็ใo VT/VR" FROM 000,000 TO 400,630 PIXEL 
...
@ 015, 005 LISTBOX oListID FIELDS HEADER "","S้rie","NF","Fornecedor","Loja","Mes","Total R$" SIZE 310,170 OF oPanelLeft PIXEL 
...
If !lOk
	MsgStop("Titulo "+cKey1+" jแ existente, informe o setor de T.I.", "Aten็ใo")
	Return
EndIf
*/
...
For nI := 1 TO LEN(aTitGer)
	...
	If lMsErroAuto
	   MsgStop("Problemas na gera็ใo do titulo "+cKey2+", informe o setor de T.I.", "Aten็ใo")
	   Return
	EndIf
...
IF CTOD(cPgto) >= dDataBase
   lRet := !lRet
ELSE
   MsgStop("Data de pgto deste lote ้ inferior a data base do sistema", "Aten็ใo")
   lRet := .F.
ENDIF   
Return lRet
