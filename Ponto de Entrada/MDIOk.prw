...
//If __cUserId == "000000"
	u_BkWeb(u_BkRest()+"/RestMsgUs/v2?userlib="+cToken)
...
oDlg := MsDialog():New( nTop, nLeft, aSize[3], aSize[4],"Avisos do Sistema",,,,,,,,, .T.,,,, .F. )
...
If AMIIn(6).AND. (u_InGrupo(__cUserId,"000024") .OR. __cUserId == "000000")
	lCP := .T.
EndIf
