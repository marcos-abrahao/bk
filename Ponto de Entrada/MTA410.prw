...
If Empty(M->C5_MDCONTR) .and. (ALTERA .OR. INCLUI)
	If Empty(M->C5_ESPECI1)
		MsgBox("C.Custo (BK) deve ser obrigatoriamente preenchido!","TI - BK","ALERT")
		_lRet := .F.
	ElseIf Empty(M->C5_XXCOMPT)
		MsgBox("Para pedidos avulsos, a competência deve ser obrigatoriamente preenchida!","TI - BK","ALERT")
		_lRet := .F.
	Else
		If VAL(SUBSTR(M->C5_XXCOMPT,1,2)) < 1 .OR. VAL(SUBSTR(M->C5_XXCOMPT,1,2)) > 12
			MsgBox("Preencha corretamente o mes da competência!","TI - BK","ALERT")
			_lRet := .F.
		EndIf	
		If _lRet .AND. (VAL(SUBSTR(M->C5_XXCOMPT,3,4)) < 2009 .OR. VAL(SUBSTR(M->C5_XXCOMPT,3,4)) > 2100)
			MsgBox("Preencha corretamente o ano da competência!","TI - BK","ALERT")
			_lRet := .F.
    	EndIf
   EndIf
EndIf
